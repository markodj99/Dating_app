...
namespace API.SignalR
{
    public class PresenceTracker
    {
        ...
        public Task<string[]> GetOnlineUsers() => Task.FromResult(OnlineUsers.Keys.OrderBy(k => k).ToArray());

        public static Task<List<string>> GetConnectionsForUser(string userId)
        {
            if (OnlineUsers.TryGetValue(userId, out var connections)) return Task.FromResult(connections.Keys.ToList());
