...
export class ToastService {
  ...
  private createToeastElement(message: string, alertClass: string,
    duratiom = 5000, avatar?:string, route?: string) {
  ...
  success(message: string, duration?: number, avatar?: string, route?: string): void {
    this.createToeastElement(message, 'alert-success', duration, avatar, route);
  }

  error(message: string, duration?: number, avatar?: string, route?: string): void {
    this.createToeastElement(message, 'alert-error', duration, avatar, route);
  }

  warning(message: string, duration?: number, avatar?: string, route?: string): void {
    this.createToeastElement(message, 'alert-warning', duration, avatar, route);
  }

  info(message: string, duration?: number, avatar?: string, route?: string): void {
    this.createToeastElement(message, 'alert-info', duration, avatar, route);
  }
}
