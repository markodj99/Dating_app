...
export class MemberList implements OnInit {
  ...
  loadMembers() {
    this.memberService.getMembers(this.memberParams).subscribe({
      next: paginatedMembers => this.paginatedMembers.set(paginatedMembers),
  ...
  get displayMessage(): string {
    ...
    if (this.updatedParams.gender) {
      const capitalizedGender = this.updatedParams.gender.charAt(0).toUpperCase() + this.updatedParams.gender.slice(1);
      filters.push(`${capitalizedGender}s`);
    }
