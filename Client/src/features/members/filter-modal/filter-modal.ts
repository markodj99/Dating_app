...
export class FilterModal {
  ...
  onMinAgeChange() {
    if (this.memberParams().minAge < 18 
      || this.memberParams().minAge > this.memberParams().maxAge) this.memberParams().minAge = 18;
  }

  onMaxAgeChange() {
    if (this.memberParams().maxAge > 100 
      || this.memberParams().maxAge < this.memberParams().minAge) this.memberParams().maxAge = 100;
  }
}
